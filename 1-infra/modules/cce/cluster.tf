...
# ---------------------------------------------------------------------------
# Security group rules on the auto-created CCE worker node SG
# ---------------------------------------------------------------------------

resource "opentelekomcloud_networking_secgroup_rule_v2" "workers_nodeport_from_elb" {
  description       = "ELB → NodePorts"
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 30000
  port_range_max    = 32767
  remote_ip_prefix  = var.ingress_subnet_cidr
  security_group_id = opentelekomcloud_cce_cluster_v3.this.security_group_node
}

resource "opentelekomcloud_networking_secgroup_rule_v2" "workers_ssh_from_bastion" {
